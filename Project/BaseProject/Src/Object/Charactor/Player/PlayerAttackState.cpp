...
void PlayerAttackState::Update(Player* player)
{
	...
	bool aaa = ins->IsMouseTrgUp(MOUSE_INPUT_LEFT);// trueにならない
	bool bbb = ins->IsMouseTrgDown(MOUSE_INPUT_LEFT);// trueになる
	bool ccc = ins->IsMouseNew(MOUSE_INPUT_LEFT);// trueになる
	if (aaa)
	{
		int a = 0;
	}
