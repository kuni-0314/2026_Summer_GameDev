...
{
    //float4 g_param; // x: ブラーの強度（0.0～1.0、中心からの距離に応じてブラーが強くなる）、y:ズーム倍率、z:フェードの進行度
    float4 g_param; // x:　エフェクトの進行度
}
...
{
    // ズーム倍率
    float zoom = 1.0f + g_param.x;

    // ズーム後のUV
    float2 zoomUV = (PSInput.uv - CENTER) / zoom + CENTER;
    
    // 画面中心から現在のピクセルへの方向ベクトルを計算
    // この方向に沿ってサンプリングを行う
    float2 dir = zoomUV - CENTER;
    ...
    float4 finalColor = lerp(col, white, fadeIntensity);
